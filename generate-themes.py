...
for color in os.listdir("src/Mint-X/variations"):
    ...
    if os.path.isdir(path):
        ...
        accent_files.append(os.path.join(theme, "libadwaita", "defaults-light.css"))
        accent_files.append(os.path.join(theme, "libadwaita", "defaults-dark.css"))
...
for color in y_hex_colors1.keys():
    for variant in ["", "-Dark"]:
        ...
        if os.path.isdir(path):
            ...
            files.append(os.path.join(theme, "libadwaita", "defaults-light.css"))
            files.append(os.path.join(theme, "libadwaita", "defaults-dark.css"))
            for file in files:
                if os.path.exists(file):
                    for accent in Y_HEX_ACCENT1:
